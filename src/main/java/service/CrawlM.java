...
public class CrawlM {
    ...
    private final int fundID_index = 0;

    private String url = "http://www.jisilu.cn/data/sfnew/fundm_list/?___t=";
    private File fileTemp = null;

    public CrawlM(File fileTemp) {
    ...
    public void parsePage(String text, String timeFormat) {
        ...
        while (matcher.find()) {
            ...
            if (b) {
                ...
                System.out.println("M类：更改条目"+id + "==" + list1.toString());
            }
        }
        if (count>0) {
            write2AllProperties(fileTemp, map, false);
            System.out.println("解析M类基金页面中...... 共成功得到记录：" + count);
        }
    }

    public static void main(String[] args) {
        CrawlM crawlM = new CrawlM(new File("tempFile"+System.getProperty("file.separator")+"tableM_temp.properties"));
